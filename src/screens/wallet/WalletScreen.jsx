...
export default function WalletScreen({navigation}) {
  ...
  const authCountry = useSelector(
    s => s.auth?.user?.address?.country || s.auth?.user?.country || null,
  );
  const appCountry = useSelector(s => s.app?.country || null);
  const country = useMemo(
    () => authCountry || appCountry || null,
  ...
  const fetchSubscription = useCallback(async () => {
    ...
    try {
      ...
      if (__DEV__ && nextSubscription?.status) {
        const statusText = String(nextSubscription.status).toLowerCase();
        if (statusText === 'cancelled') {
          try {
            console.log('[Wallet] Fetched cancelled subscription', {
              status: nextSubscription.status,
              subscriptionId: nextSubscription.id || nextSubscription._id,
              cancelledAt: nextSubscription.razorpaySubscriptionCancelledAt,
              endsAt: nextSubscription.razorpaySubscriptionEndsAt,
              legacyCancelledAt: nextSubscription.cancelledAt,
              legacyEndsAt: nextSubscription.endsAt,
            });
          } catch (logErr) {
            console.log(
              '[Wallet] Fetched cancelled subscription (raw)',
      ...
      if (err?.status === 404) {
        ...
        setSubscriptionError(
          err?.message || 'Unable to load membership details.',
        );
      }
    } finally {
      ...
      const mapped = (Array.isArray(list) ? list : [])
        .map(p => {
          ...
          const image =
            ...
            p?.coverImage ||
            p?.image ||
            'https://via.placeholder.com/96x96.png?text=Z';
          ...
          const verificationStatus = String(
            p?.fulfillment?.verificationStatus || '',
          ).toUpperCase();
          let status = 'pending';
          if (verificationStatus === 'VERIFIED') status = 'verified';
          else if (
            ['APPROVED', 'PAID', 'SUCCESS', 'COMPLETED'].includes(rawStatus)
          )
            status = 'approved';
          else if (
            ['REJECTED', 'FAILED', 'DECLINED', 'CANCELLED'].includes(rawStatus)
          )
            status = 'rejected';
          const withdrawAmount = Number(
            p?.fulfillment?.walletCredit?.amount ?? 0,
          );
          const updatedAt =
            p?.updatedAt || p?.modifiedAt || p?.createdAt || null;
          return {
            id,
            title,
            amount,
  ...
  const limitedWithdrawals = useMemo(
    () => (withdrawals || []).slice(0, VISIBLE_WITHDRAW_COUNT),
    [withdrawals],
  );
  const hasMoreWithdrawals = useMemo(
    () => (withdrawals || []).length > VISIBLE_WITHDRAW_COUNT,
    [withdrawals],
  );
    ...
    async plan => {
      try {
        ...
        if (!isGatewaySupportedForCountry(gateway, country)) {
          Alert.alert(
            'Plan not available',
            'This pack is not available in your region yet.',
          );
          return;
        }
        ...
        if (gateway === 'razorpay') {
          try {
            console.log(
              '[TOPUP] Using Razorpay for plan:',
              JSON.stringify({id: plan._id, gateway}),
            );
          } catch {}
          ...
          try {
            console.log(
              '[RZP][TOPUP] Public key fetched:',
              keyId ? `${String(keyId).slice(0, 6)}…` : null,
            );
          } catch {}
          if (!keyId) throw new Error('Missing Razorpay key');

          const orderRes = await paymentsService.createRazorpayTopup({
            planId: plan._id,
          });
          const {order} = orderRes || {};
          try {
            console.log(
              '[RZP][TOPUP] Order created:',
              JSON.stringify({
                id: order?.id,
                amount: order?.amount,
          ...
          try {
            ...
            try {
              ...
              setCongratsMsg(
                coins !== '0'
                  ? `Your ${coins} ZishCoins are on the way!`
        ...
        try {
          console.log(
            '[TOPUP] Using Stripe for plan:',
            JSON.stringify({id: plan._id, gateway}),
          );
        } catch {}
        const intentRes = await paymentsService.createStripeTopupIntent({
          planId: plan._id,
        });
        const clientSecret =
          intentRes?.clientSecret || intentRes?.client_secret;
        ...
        const {error: initError} = await initPaymentSheet({
          ...
          defaultBillingDetails: Object.keys(defaultBillingDetails).length
        ...
        setPayError(
          e?.message || 'We could not start the payment. Please retry.',
        );
      } finally {
        setPayProcessing(false);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView
        ...
        <View style={styles.row2}>
          <TouchableOpacity
            ...
            <LinearGradient
              ...
              style={[styles.balanceCard, {marginRight: 8}]}>
              <View style={styles.coinBadge}>
                <Text style={styles.coinBadgeText}>Z</Text>
              </View>
              ...
              {loading ? (
                <>
                  ...
                  <View
                    style={[styles.skelLineSm, {marginTop: 8, width: 70}]}
                  />
                  <View
                    style={[styles.skelLineXs, {marginTop: 10, width: '90%'}]}
                  />
                </>
              ) : (
                <>
                  <Text style={styles.balanceQty}>
                    {wallet.availableZishCoins.toLocaleString()}
                  </Text>
                  <Text style={styles.balanceUnit}>Coins</Text>
                  <Text style={styles.balanceCaption}>
                    Use for gameplay and entries only. Non-withdrawable.
                  </Text>
                </>
              )}
            </LinearGradient>
          ...
          <TouchableOpacity
            ...
            <View
              style={[
                styles.balanceCard,
                {backgroundColor: '#16a085', marginLeft: 8},
              ]}>
              <View style={styles.coinBadge}>
                <Text style={styles.coinBadgeText}>Z</Text>
              </View>
              <Text style={[styles.balanceTitle, {marginTop: 8}]}>
                CASH WITHDRAWAL
              </Text>
              {loading ? (
                <>
                  ...
                  />
                  <View
                    style={[styles.skelLineXs, {marginTop: 10, width: '95%'}]}
                  />
                </>
              ) : (
                <>
                  <Text style={styles.balanceQty}>
                    {wallet.withdrawalBalance.toLocaleString()}
                  </Text>

                  <Text style={styles.balanceCaption}>
                    Available to withdraw subject to T&C.
                  </Text>
                </>
              )}
            </View>
        ...
        {!!error && (
          <Text style={{color: '#FF7A7A', paddingHorizontal: 12, marginTop: 8}}>
            {error}
          </Text>
        )}
            ...
            <ScrollView
              ...
              {plansLoading && (!topupPlans || topupPlans.length === 0)
                ? [1, 2, 3].map(i => (
                    <View
                      key={`sk-${i}`}
                      style={[styles.packCard, i > 1 && {marginLeft: 12}]}>
                      <View
                        style={[
                          styles.skelLineSm,
                          {
                      ...
                      <View
                        style={[
                          styles.skelLineSm,
                          {marginTop: 8, width: '70%', alignSelf: 'center'},
                        ]}
                      />
                      <View
                        style={[
                          styles.skelLineXs,
                          {marginTop: 6, width: '50%', alignSelf: 'center'},
                        ]}
                      />
                      <View
                        style={[
                          styles.skelLineXs,
                          {marginTop: 10, width: '80%', alignSelf: 'center'},
                        ]}
                      />
                    </View>
                  ))
                : (topupPlans && topupPlans.length ? topupPlans : []).map(
                    (p, i) => (
                      <View
                        key={p._id || i}
                        style={[styles.packCard, i > 0 && {marginLeft: 12}]}>
                        <View style={styles.packBadge}>
                          <Text style={styles.coinBadgeText}>Z</Text>
                        </View>
                        <Text style={styles.packQty}>
                          {Number(p.coins || 0).toLocaleString()} Coins
                        </Text>
                        <Text style={styles.packPrice}>
                          {p.currencyCode || p.baseCurrency || ''} {p.amount}
                        </Text>
                        <Button
                          title={
                            processingPlanId === p._id
                              ? 'Processing…'
                              : 'Buy Now'
                          }
        ...
        <View style={styles.panel}>
          {withdrawalsLoading ? (
            [1, 2, 3].map(i => (
              <View
                key={`wd-sk-${i}`}
                style={[styles.withdrawRow, i > 1 && styles.rowDivider]}>
                <View
                  style={[
                    styles.thumb,
                ...
                <View style={{flex: 1}}>
                  ...
                  <View
                    style={[styles.skelLineXs, {width: '50%', marginTop: 6}]}
                  />
                </View>
            ...
            limitedWithdrawals.map((w, idx) => {
              const over =
                String(
                  w?.raw?.tournament?.status || w?.raw?.tournamentStatus || '',
                ).toUpperCase() === 'OVER';
              const rc = w?.raw?.fulfillment?.receiverConfirmation || {};
              const receiverApproved = !!(
                rc?.confirmedAt ||
                rc?.confirmed ||
                (rc?.status && String(rc.status).toUpperCase() === 'CONFIRMED')
              );
              const verificationStatus = String(
                w?.raw?.fulfillment?.verificationStatus || '',
              ).toUpperCase();
              const isVerified = verificationStatus === 'VERIFIED';
              const approvalStatus = String(
                w?.raw?.approvalStatus || '',
              ).toUpperCase();
              const rejectionReason = w?.raw?.rejectionReason;
              return (
                <View
                  key={w.id}
                  style={[styles.withdrawRow, idx > 0 && styles.rowDivider]}>
                  <Image source={{uri: w.image}} style={styles.thumb} />
                  <View style={{flex: 1}}>
                    <Text style={styles.withdrawTitle} numberOfLines={2}>
                      {w.title}
                    </Text>
                  ...
                  <View style={styles.withdrawActions}>
                    ...
                    {isVerified ? (
                      <TouchableOpacity
                        style={[
                          styles.smallBtn,
                          {backgroundColor: '#16a085', marginTop: 6},
                        ]}
                        onPress={() =>
                          navigation.navigate('Withdraw', {
                            maxAmount: wallet.withdrawalBalance,
                            productId: w.id,
                          })
                        }>
            ...
            <Text
              style={{
                color: withdrawalsError ? '#FF7A7A' : colors.textSecondary,
                textAlign: 'center',
                paddingVertical: 8,
              }}>
        ...
        <View style={styles.panel}>
          {ledgerLoading && history.length === 0 ? (
            [1, 2, 3].map(i => (
              <View
                key={`txn-sk-${i}`}
                style={[styles.txnRow, i > 1 && styles.rowDivider]}>
                <View style={{flex: 1}}>
                  <View style={[styles.skelLineSm, {width: '60%'}]} />
                  <View
                    style={[styles.skelLineXs, {width: '40%', marginTop: 6}]}
                  />
                </View>
                <View style={[styles.skelLineSm, {width: 70}]} />
              </View>
            ...
            history.map((h, idx) => (
              <View
                key={h.id}
                style={[styles.txnRow, idx > 0 && styles.rowDivider]}>
                <View style={{flex: 1}}>
                ...
                <Text style={[styles.txnDelta, {color: h.color}]}>
                  {h.delta} <Text style={styles.txnUnit}>{h.unit}</Text>
                </Text>
              </View>
            ))
          ) : (
      ...
      <AppModal
        ...
        message={
          payError || 'We could not process your payment. Please try again.'
        }
        cancelText="Close"
        confirmText="Retry"
        ...
        onConfirm={() => {
          setPayError(null);
          if (lastPlan) startTopup(lastPlan);
        }}
      />
...
function MembershipCard({subscription, loading, error, onManage}) {
  const statusInfo = categorizeSubscriptionStatus(
    subscription?.status,
    subscription,
  );
  const active = statusInfo.state === 'active';
  const cancelled = statusInfo.state === 'cancelled';
  const scheduledCancel = statusInfo.state === 'scheduled_cancel';

  const planName =
    subscription?.plan?.name || subscription?.plan_id || 'Zishes Membership';
  const title = active
    ? planName
    : cancelled
  ...
  const cancellationRequestedAt = formatSubscriptionDate(
  ...
  let credits = null;
  if (subscription?.plan?.coins != null) {
  ...
  const planPricing = formatPlanPricing(subscription?.plan);

  let body =
    'Access exclusive benefits and faster support with our membership tiers. Top-ups will be available in your region soon.';
  ...
  if (loading) {
    ...
    const base = statusLabel
      ? `Your membership is ${statusLabel.toLowerCase()}.`
      : 'Your membership is active.';
    body = renewalDate ? `${base} Renews on ${renewalDate}.` : base;
  } else if (scheduledCancel) {
    body = accessUntilDate
      ? `Auto-renew will stop on ${accessUntilDate}. You keep your benefits until then.`
      : 'Auto-renew will stop after this billing cycle.';
    if (cancellationRequestedAt) {
      body = `${body} Cancellation requested on ${cancellationRequestedAt}.`;
    }
  } else if (cancelled) {
    body = accessUntilDate
      ? `Your membership is cancelled. Benefits remain until ${accessUntilDate}.`
      : 'Your membership is cancelled. You can resubscribe anytime to restore benefits.';
  }

  let buttonLabel = 'Buy Subscription';
  if (active || scheduledCancel) buttonLabel = 'Manage Membership';
  else if (cancelled) buttonLabel = 'Resubscribe';

  const showMeta = active || cancelled || scheduledCancel;
    ...
    <View style={styles.membershipCard}>
      ...
      {showMeta ? (
        <View style={styles.membershipMeta}>
          ...
          {(cancelled || scheduledCancel) && accessUntilDate ? (
            <Text style={styles.membershipMetaText}>
              Access until: {accessUntilDate}
            </Text>
          ) : null}
          {scheduledCancel && cancellationRequestedAt ? (
            <Text style={styles.membershipMetaText}>
              Cancellation requested: {cancellationRequestedAt}
            </Text>
          ) : null}
          {credits ? (
            <Text style={styles.membershipMetaText}>{credits}</Text>
          ) : null}
          {planPricing ? (
            <Text style={styles.membershipMetaText}>{planPricing}</Text>
          ) : null}
        </View>
      ) : null}
      <Button
      ...
      />
      {showMeta ? null : (
        <Text style={styles.membershipFooter}>
          Membership availability depends on your region.
        </Text>
      )}
    </View>
  );
...
function formatPlanPricing(plan) {
  ...
  const formattedAmount = hasAmount
    ? `${currency ? `${currency} ` : ''}${rawAmount.toLocaleString('en-IN')}`
    : null;

  const periodRaw = plan?.billingPeriod
    ? String(plan.billingPeriod).replace(/_/g, ' ').toLowerCase()
    : null;
  const interval = Number(plan?.billingInterval || 1);
  let frequency = null;
  if (periodRaw) {
    const normalizedPeriod = periodRaw.replace(/(^|\s)\w/g, c =>
      c.toUpperCase(),
    );
    frequency =
      interval > 1 ? `${interval} ${normalizedPeriod}` : normalizedPeriod;
  }
...
function ActionButton({status, onPress}) {
  ...
  return (
    <TouchableOpacity
      style={[styles.smallBtn, {backgroundColor: bg}]}
      onPress={onPress}>
